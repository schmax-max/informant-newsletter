...
console.log('starting processLatest')
    try {
        ...
        return { archive_date, archive_url, links }
    } catch(e) {
        console.log({e})
        return e
    }
...
function getLinks (source, apiResponse) {
    ...
    if (source.source_url.includes('https://getpocket.com/explore/pocket-hits')) {
...
function getRelevantApiResponse (source, apiResponse) {
    const {beginning_identifier, end_identifier} = source.filtering
...
async function getRecommendationDate (apiResponse, archive_url) {
  ...
  if (date) {
    const dateConfig = {
        project: `helper-date`, 
        data: {date, url: archive_url},
        type: 'informant-newsletter'
    }
    return await postData (dateConfig)
  } else {
    return 
  }
}
