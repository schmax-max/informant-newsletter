...
//   console.log({source_url})
  const apiResponse = await getApiResponse (source_url)
  ...
  if (!archiveLinkChunks) {
      console.log(`no archive link chunks for ${source_url}`)
  }
  if (!archiveLinks) {
      console.log(`no archive links for ${source_url}`)
  }
...
async function getArchiveLinks (archiveLinkChunks, source) {
  ...
  for (let i=0; i<archiveLinkChunks.length; i++) {
      ...
      let {date, url} = await getDateAndUrl (source_url, item)
  ...
  if (linkArray.length === 0) {
      const errorData = {source_url, archive_identifier_slug}
...
async function getDateAndUrl (source_url, item) {
  ...
  if (source_url.includes('densediscovery.com')) {
      url = itemArray[0]
  } else if (source_url.includes('nowiknow.com')) {
      url = itemArray[3]
      date = itemArray[6].split('</')[0]
      date = date.split('>')[1]
      fetchDate = true
  } else if (source_url.includes('tinyletter.com')) {
      url = itemArray[3]
      date = itemArray[0].split('</')[0]
      fetchDate = true
  } else if (source_url.includes('ribbonfarm.com')) {
      url = itemArray[0]
      date = itemArray[6]
      fetchDate = true
  } else if (source_url.includes('getrevue.co')) {
      url = itemArray[6]
  ...
  } else if (source_url.includes('aiweekly.co/issues')) {
...
function identifyLatestUnprocessed (source, archiveLinks) {
  ...
  if (
      ...
      console.log(`all links already processed for ${source.source_url}`)
      response = 'completed'
  }
